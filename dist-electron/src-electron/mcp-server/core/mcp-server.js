...
class MCPServer extends events_1.EventEmitter {
    ...
    registerSearchTools() {
        this.registerTool({
            ...
            handler: async (input) => this.executeWebSearch(input.query, input.provider ?? 'auto', input.limit ?? 10),
        });
        this.registerTool({
            ...
            handler: async (input) => this.executeSemanticSearch(input.query, input.searchSpace ?? 'web'),
        });
        this.registerTool({
            name: 'site_search',
            ...
            handler: async (input) => this.executeSiteSearch(input.site, input.query),
        });
    }
    ...
    registerBrowserTools() {
        this.registerTool({
            ...
            handler: async (input, ctx) => ctx.browserManager?.navigate(input.url, { tabId: input.tabId, waitForLoad: input.waitForLoad ?? true }),
        });
        this.registerTool({
            name: 'click',
            ...
            handler: async (input, ctx) => ctx.browserManager?.click(input.selector, input.tabId),
        });
        this.registerTool({
            name: 'type',
            ...
            handler: async (input, ctx) => ctx.browserManager?.type(input.selector, input.text, input.tabId),
        });
        this.registerTool({
            name: 'screenshot',
            ...
            handler: async (input, ctx) => ctx.browserManager?.screenshot(input.tabId, { fullPage: input.fullPage ?? false }),
        });
        this.registerTool({
            name: 'execute_script',
            ...
            handler: async (input, ctx) => ctx.browserManager?.executeScript(input.script, input.tabId),
        });
    }
    ...
    registerExtractionTools() {
        ...
        this.registerTool({
            ...
            handler: async (input, ctx) => ctx.browserManager?.extractTable(input.selector, input.tabId),
        });
        this.registerTool({
            name: 'extract_links',
            ...
            handler: async (input, ctx) => ctx.browserManager?.extractLinks(input.tabId, input.filter),
        });
    }
    // ── Workflow tools ───────────────────────────────────────────────────────────
    registerWorkflowTools() {
        this.registerTool({
            ...
            handler: async (input, ctx) => ctx.workflowEngine?.executeWorkflow(input.workflowId, input.input),
        });
        this.registerTool({
            name: 'crawl',
            ...
            handler: async (input, ctx) => ctx.crawlerService?.startCrawl({
