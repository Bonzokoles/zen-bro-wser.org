...
class NetworkManager extends events_1.EventEmitter {
    ...
    setupNetworkInterception() {
        ...
        electron_1.session.defaultSession.webRequest.onCompleted(filter, (details) => {
            this.emit('response', {
                url: details.url,
        ...
        electron_1.session.defaultSession.webRequest.onErrorOccurred(filter, (details) => {
            ...
            if (benign.some(code => details.error.includes(code)))
                return;
    ...
    async setProxy(proxyUrl, type = 'http') {
        try {
    ...
    async clearProxy() {
        await electron_1.session.defaultSession.setProxy({ proxyRules: 'direct://' });
        this.currentProxy = null;
        this.emit('proxy-changed', { proxy: null });
    }
    allowLocalhost(enabled = true) {
